...
resource "aws_iam_role" "cluster" {
  name = "test-eks-cluster-role-${random_id.role.id}"
...
resource "aws_iam_role" "node" {
  name = "test-eks-node-role-${random_id.role.id}"
  assume_role_policy = <<POLICY
{
  "Version": "2012-10-17",
  "Statement": [
...
resource "aws_iam_instance_profile" "node" {
  name = "terraform-eks-demo-eks-node-instance-profile-${random_id.role.id}"
  role = aws_iam_role.node.name
}

resource "aws_iam_policy" "AWSLoadBalancerControllerIAMPolicy" {
  name        = "AWSLoadBalancerControllerIAMPolicy-${random_id.role.id}"
  path        = "/"
...
resource "aws_iam_policy" "AWSVisualEditorPolicy" {
  name        = "AWSVisualEditorPolicy-${random_id.role.id}"
  path        = "/"
  description = "Policy for AWS Load Balancer Controller for k8s Ingress"
  policy = <<POLICY
{
